...
namespace IBM.Watsson.Examples
{
    public class ExampleStreaming : MonoBehaviour
    {
        ...
        {
           if (result != null && result.results.Length > 0 && check)
            { 
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if (life.hp >= 1)
                        {
                            if (alt.transcript.Contains(wordlist.list[count]))
                            {
                                Debug.Log("correct");
                                ...
                                StartCoroutine(ShowAndHide(correct, 2.0f)); // 1 second
                            ...
                            {
                                Debug.Log("wrong");
                                StartCoroutine(ShowAndHide(wrong, 2.0f)); // 1 second
